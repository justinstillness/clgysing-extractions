#!/usr/bin/env python3
"""
MCP Traffic Extraction Script (Component 2) — v7
==================================================
...
v7 changes: BRANCH-aware routing (D-044)
  - BRANCH field parsed from SERAPH signal string
  - BRANCH becomes top-level routing directory
  - Routing hierarchy:
  ...
  - Updated SERAPH Signal String Reference compat

v6 changes: Failure resilience (ghost files, failure taxonomy)
...
PASS_TYPE_NAMES = {
    ...
    'RX': 'Relationship Extraction', 'DM': 'Domain Mapping', 'HR': 'Harmonization',
...
def is_known_pass(code):
    return code in PASS_TYPE_NAMES or '-' in code
...
def parse_seraph_string(query_text):
    ...
    has_legacy = all(k in fields for k in ['PROJECT', 'PASS', 'NUM'])
    if not has_branch and not has_legacy:
        return None
    return fields

def get_routing_tier(seraph):
    if seraph is None:
        return ('tier3', 'Tier 3 (Untagged)')
    branch = seraph.get('BRANCH', '')
    project = seraph.get('PROJECT', '')
    pass_code = seraph.get('PASS', '')
    if branch and project and pass_code:
        if is_known_pass(pass_code):
            return ('tier1a', f"Tier 1a ({branch}/{project}/{pass_code})")
        else:
            return ('tier2', f"Tier 2 (Unknown PASS: {pass_code})")
    if branch:
        return ('tier1b', f"Tier 1b ({branch}/_unsorted)")
    if project and pass_code:
        if is_known_pass(pass_code):
            return ('tier1c', f"Tier 1c ({project}/{pass_code})")
        else:
            return ('tier2', f"Tier 2 (Unknown PASS: {pass_code})")
    return ('tier2', 'Tier 2 (Insufficient routing fields)')
...
def classify_pass_type(pass_code):
    if not pass_code: return ('unknown', 'N/A')
    if pass_code in PASS_TYPE_NAMES: return ('known', PASS_TYPE_NAMES[pass_code])
    if '-' in pass_code: return ('known', pass_code)
...
def _build_metadata(call, seraph, meta_match, answer, failure_info=None):
    ...
    if seraph:
        pass_code = seraph.get('PASS', '')
        _, pass_name = classify_pass_type(pass_code)
        tier_name, tier_label = get_routing_tier(seraph)
        meta.update({
            'tag_type': 'seraph', 'branch': seraph.get('BRANCH', ''),
            'project': seraph.get('PROJECT', ''), 'pass_code': pass_code,
            'num': seraph.get('NUM', ''), 'sp': seraph.get('SP', ''),
            'theme': seraph.get('THEME', ''), 'session': seraph.get('SESSION', ''),
            'config': seraph.get('CONFIG', 'default'), 'thread': seraph.get('THREAD', ''),
            'notes': seraph.get('NOTES', ''), 'tier_name': tier_name,
            'tier_label': tier_label, 'pass_name': pass_name,
            'clean_query': strip_seraph_string(query_text)})
    elif meta_match:
        meta.update({'tag_type': 'legacy_meta', 'branch': '', 'sp': meta_match.group('sp'),
            'pass_code': meta_match.group('ptype'), 'num': meta_match.group('seq'),
            'theme': meta_match.group('desc'), 'project': '',
            'pass_name': PASS_TYPE_NAMES.get(meta_match.group('ptype'), meta_match.group('ptype')),
            'tier_name': 'legacy', 'tier_label': 'Legacy META format (unrouted)',
            'clean_query': META_PATTERN.sub('', query_text).strip()})
    else:
        meta.update({'tag_type': 'untagged', 'branch': '', 'project': '',
            'tier_name': 'tier3', 'tier_label': 'Tier 3 (Untagged)',
            'clean_query': query_text})
    return meta

def _build_filename(meta, is_ghost=False):
    ts_str = meta['ts_str']
    sfx = '_FAILED' if is_ghost else ''
    if meta['tag_type'] == 'seraph':
        project = meta.get('project', '')
        pass_code = meta.get('pass_code', '')
        num = meta.get('num', '')
        sp = meta.get('sp', '')
        parts = [ts_str]
        if project: parts.append(project)
        if sp: parts.append(f"SP{sp}")
        if pass_code and num: parts.append(f"{pass_code}-{num}")
        elif pass_code: parts.append(pass_code)
        return '_'.join(parts) + f"{sfx}_notebook-query.txt"
...
def _resolve_dest_dir(meta, extracted_dir, is_ghost=False):
    tier = meta.get('tier_name', 'tier3')
    branch = meta.get('branch', '')
    project = meta.get('project', '')
    pass_code = meta.get('pass_code', '')
    if is_ghost:
        if tier == 'tier1a': return extracted_dir / '_failed' / branch / project / pass_code
        elif tier == 'tier1b': return extracted_dir / '_failed' / branch
        elif tier == 'tier1c': return extracted_dir / '_failed' / project / pass_code
        else: return extracted_dir / '_failed'
    if tier == 'tier1a': return extracted_dir / branch / project / 'phase2' / pass_code
    elif tier == 'tier1b': return extracted_dir / branch / '_unsorted'
    elif tier == 'tier1c': return extracted_dir / project / 'phase2' / pass_code
    elif tier == 'tier2':
        if branch: return extracted_dir / branch / '_review'
        return extracted_dir / '_review'
    else: return extracted_dir / '_untagged'
...
def _build_header(meta):
    ...
    if meta['tag_type'] == 'seraph':
        ...
        return (
            f"Branch:          {branch if branch else 'N/A'}\n"
            f"Source Package:  {sp_d}\n"
            f"Pass Type:       {meta.get('pass_code', 'N/A') or 'N/A'} ({meta.get('pass_name', 'N/A')})\n"
            f"Pass Number:     {meta.get('num', 'N/A') or 'N/A'}\n"
            f"Theme/Focus:     {meta.get('theme') or 'N/A'}\n"
            f"Project:         {meta.get('project', 'N/A') or 'N/A'}\n"
            f"Session:         {meta.get('session') or 'N/A'}\n"
            f"Routing:         {meta['tier_label']}\n"
            f"Query:           {meta['clean_query']}\n"
            f"Timestamp:       {meta['timestamp_iso']}\n"
            f"Notebook ID:     {meta['notebook_id']}\n"
            f"Source IDs:      {meta['source_ids_str']}\n"
            f"Conversation ID: {meta['conversation_id']}\n"
            f"Chat Configure:  {meta.get('config', 'default')}\n"
            f"Thread:          {meta.get('thread') or 'N/A'}\n"
            f"Notes:           {meta.get('notes') or 'N/A'}\n"
            f"Response Chars:  {meta['response_chars']}\n"
            f"Duration:        {meta['duration']}ms")
    elif meta['tag_type'] == 'legacy_meta':
        return (
            f"Branch:          N/A\n"
            f"Source Package:  {meta['sp']}\n"
            f"Pass Type:       {meta['pass_code']} ({meta['pass_name']})\n"
            f"Pass Number:     {meta['num']}\n"
            f"Theme/Focus:     {meta['theme']}\n"
            f"Routing:         Legacy META format (unrouted)\n"
            f"Query:           {meta['clean_query']}\n"
            f"Timestamp:       {meta['timestamp_iso']}\n"
            f"Notebook ID:     {meta['notebook_id']}\n"
            f"Source IDs:      {meta['source_ids_str']}\n"
            f"Conversation ID: {meta['conversation_id']}\n"
            f"Chat Configure:  default\n"
            f"Response Chars:  {meta['response_chars']}\n"
            f"Duration:        {meta['duration']}ms")
    else:
        return (
            f"Branch:          N/A\n"
            f"Source Package:  N/A\n"
            f"Pass Type:       N/A (N/A)\n"
            f"Pass Number:     N/A\n"
            f"Theme/Focus:     Untagged\n"
            f"Routing:         Tier 3 (Untagged)\n"
            f"Query:           {meta['clean_query']}\n"
            f"Timestamp:       {meta['timestamp_iso']}\n"
            f"Notebook ID:     {meta['notebook_id']}\n"
            f"Source IDs:      {meta['source_ids_str']}\n"
...
def write_extracted_files(tool_calls, extracted_dir):
    ...
    for call in tool_calls:
        ...
        if answer and len(answer.strip()) > 0:
            ...
            success_count += 1
            b = meta.get('branch', '') or meta.get('tier_name', 'untagged')
...
def process_log(date_str=None):
    ...
    if branch_counts:
        bc_str = ', '.join(f"{b}={c}" for b, c in sorted(branch_counts.items()))
        print(f"            Routing: {bc_str}")
    print(f"  Done.")
